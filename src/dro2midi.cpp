...
//
//  v1.6W / 2021-06-30 / ericvids (ericvids at gmail dot com): Changes
//     - Removed output level from data registers when putting into mapping file,
//       eliminating many duplicate entries. The default mapping file will need
//       to be rewritten (but a complete mapping file for Wolf3d/SOD has been
//       created, hence the W version).
//     - Added lowpatch= and lownote= options to allow a different instrument
//       patch to be used when the note is below a given pitch. This fixes a
//       problem with XEVIL.WLF, which uses the same register map for a low-pitch
//       marching sound and a high-pitch chromatic percussion sound, which
//       required two separate MIDI patches.
//     - Note lengths are now ensured to be greater than 20 msec. This fixes
//       playback of really short notes on crappy synthesizers like Microsoft
//       GS Wavetable Synth. Currently this feature only works for IMF/WLF files.
...
#define NOTE_LENGTH_RATIO   50   // notes should last at least 1/nth of a second
#define PITCHBEND_RANGE 2.0      // 2 == most compatible option
...
void version()
{
	printf("DRO2MIDI v" VERSION " - Convert raw Adlib captures to General MIDI\n"
		...
		"With contributions by ericvids (C) 2021.\n"
...
bool loadInstruments(void)
{
	...
	for (::instrcnt = 0; fgets(line, sizeof(line)-1, f) && (instrcnt < MAXINSTR);) {
		...
		while (sscanf(cList, "%s%n", nextopt, &iLen) >= 1) {
			...
			if (sscanf(nextopt, "patch=%d", &iValue) == 1) {
			...
			} else if (sscanf(nextopt, "lowpatch=%d", &iValue) == 1) {
				// MIDI low patch
				in.isdrum = 0;
				in.lowprog = iValue - 1;
				if ((in.lowprog < 0) || (in.lowprog > 127)) {
					fprintf(stderr, "ERROR: Instrument #%d (line %d) was set to "
						"lowpatch=%d, but this value must be between 1 and 128 inclusive.\n",
						instrcnt, iLineNum, in.lowprog + 1);
					return false;
				}
			} else if (sscanf(nextopt, "lownote=%d", &iValue) == 1) {
				// MIDI low note
				in.isdrum = 0;
				in.note = iValue;
				if ((in.note < 0) || (in.note > 127)) {
					fprintf(stderr, "ERROR: Instrument #%d (line %d) was set to "
						"note=%d, but this value must be between 0 and 127 inclusive.\n",
				...
				if ((in.note < 0) || (in.note > 127)) {
					fprintf(stderr, "ERROR: Drum instrument #%d (line %d) was set to "
						"drum=%d, but this value must be between 0 and 127 inclusive.\n",
						instrcnt, iLineNum, in.note);
					return false;
				}
	...
	switch (ri) {
		...
		case BassDrum:
			...
			return
				...
				difference(a.reg40[1] & 0xc0, b.reg40[1] & 0xc0, 1) +
		...
		switch (ri) {
			...
			case BassDrum:
				...
				fprintf(stderr, "%s %02X-%02X/%02X-%02X/%02X-%02X/%02X-%02X/%02X/"
					...
					reg[chanOPL].reg40[0], reg[chanOPL].reg40[1] & 0xc0,
				...
				while (delay > 0)
				{
					...
					for (int chan = 0; chan < 16; chan++)
					{
						if (keyAlreadyOn[chan])
							keyTimeOut[chan]++;
						if (keyRequestOff[chan])
							doNoteOnOff(false, chan, chan);
					}
				}
